...
class Game:
    def __init__(self, size, render_mode="human"):
        ...
        #effect
        self.spot_light = Spot_Light(self)
    ...
    def running(self):
        ...
        while running:
            ...
            self.maze.draw_final_maze(self.path)
            self.safezone.update_safe_zone(dt)
            self.player.update_player(dt)

            self.exit_point.draw_exit_point()
            for ghost in self.ghosts:
                ghost.update_ghost(dt)
            #get spotlight list
            Spot_Light_list = []
            for safe_zone in self.safezone.important_safe_zone_list:
                Spot_Light_list.append(tuple(safe_zone))
    
            Spot_Light_list.append(tuple(self.player.pixel_pos))
            
            self.spot_light.spot_light_effect(Spot_Light_list)
            pygame.display.flip()
